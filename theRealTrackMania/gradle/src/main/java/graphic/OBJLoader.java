...
public class OBJLoader {
	...
	List<String> lines = Utils.getModel(fileName);
	...
	for (String line : lines) {
	    ...
	    switch (tokens[0]) {
	        ...
	        case "f":
	        	...
	        	if(tokens.length == 4) {
	        		Tface = new TriangleFace(tokens[1], tokens[2], tokens[3]);
	        		Trfaces.add(Tface);
	        	}else {
	        		Qface = new QuadFace(tokens[1], tokens[2], tokens[3],tokens[4]);
	        		TriangleFace[] tr = Qface.ToTriangle(Qface);
	        		Trfaces.add(tr[0]);
	        		Trfaces.add(tr[1]);
