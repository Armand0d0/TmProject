...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal)
{
    ...
    return (diffuseColour+ specColour-specColour);
}
...
vec4 calcSpotLight(SpotLight light, vec3 position, vec3 normal)
{
   ...
   // vec4 light_colour = calcLightColour(light.colour, light.pl.intensity, position, to_light_dir, normal);
	...
	if ( spot_alfa > light.angle ) {
        colour = calcPointLight(light.pl, position, normal);
        colour *= (1.0 - (1.0 - spot_alfa)/(1.0 - light.angle));
...
{
        diffuseSpecularComp += calcDirectionalLight(directionalLights[i], mvVertexPos, mvVertexNormal);
   }
}
